...
import io.wizdumb.scaffolding.random.core.RubberRandom.{MaxRecursionCountName, RecursionEnabledName, RegexStrategyName}
import io.wizdumb.scaffolding.random.core._
import io.wizdumb.scaffolding.random.exceptions.RecursionDisabledException
import io.wizdumb.scaffolding.random.predicate._
import io.wizdumb.scaffolding.random.randomizers.LongRandomizer
import io.wizdumb.scaffolding.random.support.AnnotationGenerator
import io.wizdumb.scaffolding.random.types.{CollectionTypes, ParameterizedTypeReference, TypeContainer}
...
@ComponentScan(Array("io.wizdumb.scaffolding.random.*"))
...
class RandomScalaTest extends TestCase {
    ...
    def testCustomRandomizerScan: Unit = {
        implicit var r = new RubberRandomImpl(
            Seq("io.wizdumb.scaffolding", "com.dovidkopel")
        )
...
//        logger.debug("SSN: {}", ssn1)
    ...
    def test1(): Unit = {
        ...
        val bl = r.generate(classOf[io.wizdumb.scaffolding.random.Food])
        logger.debug(mapper.writeValueAsString(bl))

        val ap = r.generate(new ParameterizedTypeReference[io.wizdumb.scaffolding.random.Blah[Buffet[java.util.UUID]]]() {})
